<template>
  <div>
    <header-sub-menus>
      <li><nuxt-link to="/moment" class="link">推荐圈子
        <svg class="icon schdule_icon_active" aria-hidden="true">
          <use xlink:href="#icon-circle_blue"></use>
        </svg>
      </nuxt-link>
      </li>
      <li><nuxt-link to="/moment/my" :class="{'active':$route.path == '/moment/my' && category_id == '-3'}" class="link">我的圈子
        <svg class="icon schdule_icon_active" aria-hidden="true">
          <use xlink:href="#icon-circle_blue"></use>
        </svg>
      </nuxt-link>
    ...
    <article v-if="$route.path == '/moment/my'" class="page-main content moment_all_wrap" id="moment_my">
      <moment-son :dataList="dataList" @reload="reload()"></moment-son>
      <no-data :show="true" v-if="dataList && dataList.length < 1"></no-data>
    </article>

    <article v-else class="page-main content moment_all_wrap" id="moment_all">
...
<script>
  ...
  export default {
    ...
    async asyncData({app, $axios, error, params}) {
      // 异步数据处理，另外该方法的第一个参数为当前页面组件的 上下文对象。

      if(params.id == 'my'){
        let res = await app.$api.circle.getCircleList({params:{category_id:'-3'}});
        return {
          categoryList: res.data.category_list,
          dataList: res.data.data_list
        }
      }else{
        let res = await app.$api.circle.getCircleList({params:{category_id:params.id}});
        return {
          categoryList: res.data.category_list,
          dataList: res.data.data_list
        }
    ...
    methods: {
      ...
      async getCircle(){
        ...
        if(this.$route.params.id == 'my'){
          res = await this.$api.circle.getCircleList({params:{category_id:'-3'}});
        }else{
          res = await this.$api.circle.getCircleList({params:{category_id:this.$route.params.id}});
        }
